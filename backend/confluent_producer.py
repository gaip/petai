...
Features:
- Datadog Observability: Tracks production throughput and latency
"""
...
try:
    ...
    print("⚠️ Datadog library not found. Metrics disabled.")
    class DummyStatsd:
...
def delivery_report(err, msg):
    ...
    if err is not None:
        ...
        statsd.increment('kafka.produce.failed', tags=[f'topic:{msg.topic()}'])
    else:
        print(f'✅ Message delivered to {msg.topic()} [{msg.partition()}] @ offset {msg.offset()}')
        statsd.increment('kafka.produce.success',
                       tags=[f'topic:{msg.topic()}', f'partition:{msg.partition()}'])
    ...
    try:
        while (time.time() - start_time) < duration_seconds:
            ...
            if data['anomaly_injected']:
                ...
                statsd.increment('pet.health.anomaly.injected',
                               tags=[f'pet:{pet_id}'])
            ...
            # Track production metrics to Datadog
            produce_latency = (time.time() - produce_start) * 1000
            statsd.histogram('kafka.produce.latency', produce_latency,
                           tags=[f'topic:{TOPIC}', f'pet:{pet_id}'])
            statsd.increment('kafka.messages.produced',
                           tags=[f'topic:{TOPIC}', f'pet:{pet_id}'])

            # Send current health metrics to Datadog
            statsd.gauge('pet.health.heart_rate', data['heart_rate'],
                       tags=[f'pet:{pet_id}'])
            statsd.gauge('pet.health.activity_score', data['activity_score'],
                       tags=[f'pet:{pet_id}'])
            statsd.gauge('pet.health.gait_symmetry', data['gait_symmetry'],
                       tags=[f'pet:{pet_id}'])
